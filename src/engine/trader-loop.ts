...
export class TraderLoop {
  ...
  start(): void { if (this.timer) return; this.running = true; void this.tick(); this.timer = setInterval(() => void this.tick(), 2_000); }
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      const scanner = new OpportunityScanner(scanConfig, new SimulatedQuoteProvider());
      const amount = BigInt(Math.max(1, Math.floor(settings.maxTradeUsd * 1_000_000)));
      const opportunities = await scanner.scan({ inputMint: "So11111111111111111111111111111111111111112", outputMint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", amountIn: amount });
      const engine = new PaperExecutionEngine();
      for (const opportunity of opportunities.slice(0, 1)) await engine.execute(opportunity);
      await getPostgresPool().query("INSERT INTO bot_events (level,event_type,payload) VALUES ($1,$2,$3)", ["info", "scanner_tick", JSON.stringify({ opportunities: opportunities.length, settings })]);
    } catch (error) {
