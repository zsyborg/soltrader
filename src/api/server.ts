...
export function startApiServer(port:number, loop:TraderLoop): ReturnType<typeof createServer> {
 const server=createServer(async(req,res)=>{try{
  ...
  if(url.pathname==="/api/bot/control"&&req.method==="POST"){
   ...
   if(body.status==="running") loop.resume(); else if(body.status==="paused") loop.pause(); else loop.stop();
  ...
  if(url.pathname==="/api/settings"&&req.method==="POST"){
   ...
   const allowed=(Object.keys(patch) as Array<keyof RuntimeSettings>).every(k=>["minProfitBps","maxSlippageBps","maxTradeUsd","maxDailyLossUsd","minLiquidityUsd","opportunityTtlMs"].includes(k));
   if(!allowed)return json(res,400,{error:"Unsupported setting"});
   const settings=await setRuntimeSettings(patch);
   await getPostgresPool().query("INSERT INTO bot_events (level,event_type,payload) VALUES ($1,$2,$3)",["info","settings_updated",JSON.stringify(settings)]);
   const redis=await getRedisClient(); await redis.publish("soltrader:settings",JSON.stringify(settings));
